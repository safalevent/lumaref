...
from zeeref.config import CommandlineArgs
from zeeref.constants import COLORS
from zeeref import utils

if TYPE_CHECKING:
    from zeeref.scene import ZeeGraphicsScene
    from zeeref.view import ZeeGraphicsView

    _BaseItemBase = QtWidgets.QGraphicsItem
else:
    _BaseItemBase = object
...
class BaseItemMixin(_BaseItemBase):
    ...
    def zee_scene(self) -> ZeeGraphicsScene | None:
        ...
        if scene is None:
            return None
        return cast("ZeeGraphicsScene", scene)
    ...
    def zee_view(self) -> ZeeGraphicsView | None:
        ...
        if not views:
            return None
        return cast("ZeeGraphicsView", views[0])
    ...
    @property
    def center_scene_coords(self) -> QtCore.QPointF:
...
class SelectableMixin(BaseItemMixin):
    ...
    SELECT_LINE_WIDTH: int = 4  # line width for the selection box
    SELECT_HANDLE_SIZE: int = 15  # size of selection handles for scaling
    SELECT_RESIZE_SIZE: int = 20  # size of hover area for scaling
    SELECT_ROTATE_SIZE: int = 10  # size of hover area for rotating
    SELECT_FREE_CENTER: int = 20  # size of handle-free area in the center
    ...
    scale_orig_factor: float
    rotate_orig_degrees: float
    rotate_start_angle: float

    def has_selection_outline(self) -> bool:
        """Whether the selection outline should be drawn. Overridden by subclasses."""
        return False

    def has_selection_handles(self) -> bool:
        """Whether the selection handles should be drawn. Overridden by subclasses."""
        return False

    def selection_action_items(self) -> list[Any]:
        """The items affected by selection actions. Overridden by subclasses."""
        return [self]

    def on_selected_change(self, value: Any) -> None:
        """Called when selection state changes. Overridden by subclasses."""

    def init_selectable(self) -> None:
    ...
    def select_handle_free_center(self) -> QtCore.QRectF:
        ...
        return QtCore.QRectF(
            self.center.x() - size / 2, self.center.y() - size / 2, size, size
        )

    def draw_debug_shape(
        self,
        painter: QtGui.QPainter,
        shape: QtCore.QRectF | QtGui.QPainterPath,
        r: int,
        g: int,
        b: int,
    ) -> None:
    ...
    def paint_debug(
        ...
        if commandline_args.debug_handles and self.has_selection_handles():
            for corner in self.corners:
                self.draw_debug_shape(painter, self.get_scale_bounds(corner), 0, 0, 255)
    ...
    def corners(
        self,
    ) -> tuple[QtCore.QPointF, QtCore.QPointF, QtCore.QPointF, QtCore.QPointF]:
    ...
    @property
    def corners_scene_coords(self) -> list[QtCore.QPointF]:
    ...
    def get_scale_bounds(
        self, corner: QtCore.QPointF, margin: float = 0
    ) -> QtGui.QPainterPath:
    ...
    def get_rotate_bounds(self, corner: QtCore.QPointF) -> QtGui.QPainterPath:
    ...
    def mousePressEvent(self, event: QtWidgets.QGraphicsSceneMouseEvent | None) -> None:
        ...
        if event.button() == Qt.MouseButton.LeftButton and self.has_selection_handles():
            for corner in self.corners:
                ...
                if self.get_scale_bounds(corner).contains(event.pos()):
                    ...
                    self.event_anchor = self.mapToScene(self.get_scale_anchor(corner))
    ...
    def get_scale_factor(self, event: QtWidgets.QGraphicsSceneMouseEvent) -> float:
    ...
    def get_rotate_angle(self, pos: QtCore.QPointF) -> float:
    ...
    def get_rotate_delta(self, pos: QtCore.QPointF, snap: bool = False) -> float:
    ...
    def get_corner_scale_cursor(self, corner: QtCore.QPointF) -> Qt.CursorShape:
    ...
    def get_diag_cursor(self, is_topleft_or_bottomright: bool) -> Qt.CursorShape:
...
class MultiSelectItem(SelectableMixin, QtWidgets.QGraphicsRectItem):
    ...
    def fit_selection_area(self, rect: QtCore.QRectF) -> None:
...
class RubberbandItem(BaseItemMixin, QtWidgets.QGraphicsRectItem):
    ...
    def fit(self, point1: QtCore.QPointF, point2: QtCore.QPointF) -> None:
