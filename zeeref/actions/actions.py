...
from zeeref.actions.menu_structure import menu_structure
...
class Action:
    ...
    def __init__(
        ...
        settings_events.restore_keyboard_defaults.connect(self.on_restore_defaults)
    ...
    def get_shortcuts(self):
        return self.kb_settings.get_list(self.SETTINGS_GROUP, self.id, self.shortcuts)

    def set_shortcuts(self, value):
        logger.debug(f'Setting shortcut "{self.id}" to: {value}')
        self.kb_settings.set_list(self.SETTINGS_GROUP, self.id, value, self.shortcuts)
        if self.qaction:
