...
class ZeeSettingsEvents(QtCore.QObject):
...
# ZeeSettings instance. Since we can't instantiate ZeeSettings
# globally on module level (because the Qt app doesn't exist yet), we
# use this events proxy
settings_events = ZeeSettingsEvents()


class ZeeSettings(QtCore.QSettings):
    ...
    def valueOrDefault(self, key):
        ...
        opposed to settings that ZeeRef stores on its own.)
